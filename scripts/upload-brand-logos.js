...
const VARIANTS = [
  {
    column: 'logo_url',
    file: 'Property-Ecellence Award Logo-Dark.png',
    expect: 'gold text — sits on the dark navy header',
  ...
  },
  {
    ...
    file: 'Property-Ecellence Award Logo_Light.png',
...
async function main() {
  ...
  await pool.query(
    ...
    [urls.logo_url, urls.logo_url_light, SITE_ID],
  );

  const after = (
    await pool.query('SELECT logo_url, logo_url_light FROM sites WHERE id = $1', [SITE_ID])
  ).rows[0];
  console.log('\nRow now reads:');
