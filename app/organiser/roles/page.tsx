...
import { ManageCeremonyRolesClient } from '@/components/organiser/ManageCeremonyRolesClient';
import type { QueryResultRow } from 'pg';

export const dynamic = 'force-dynamic';
export const metadata = { title: 'Ceremony Roles' };

type EventRow = { id: number; title: string } & QueryResultRow;

export default async function CeremonyRolesPage() {
  ...
  return (
    <div className="min-h-screen bg-ink grain pt-24">
      <div className="container-luxe section-pad py-12">
        ...
        <div className="mb-8">
          <h1 className="font-display text-3xl font-semibold text-white">Ceremony Roles</h1>
          <p className="mt-1 text-white/50">
            Assign and manage all personnel for your award night — performers, production crew,
            hospitality, photography, security and honorary roles.
          </p>
        </div>
        ...
        {events.length === 0 ? (
          ...
          <ManageCeremonyRolesClient events={events} />
        )}
      </div>
    </div>
  );
